...
rules at a time before running out of memory. We have not run the whole grammar with this setup, but among the first rules we tested, it has found a few conflicts.
An example follows:
...
\noindent The problem is that the set \texttt{POSTPOSIZIOAK-9} contains the word form ``barrena'', and the other set contains the tag \texttt{INE}; hence the latter rule is redundant.
...
to any morphological tags, so allowing some hundreds or thousands of lexical forms does not pose any problems.
...
the conflict lies in the 3rd-level tag \texttt{INE}. But this approximation found successfully 11 duplicates or near-duplicates in the whole grammar, such as the following:
...
\section{Wishlist for future work}
